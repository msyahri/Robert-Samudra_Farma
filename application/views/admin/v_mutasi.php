...
<body>
    ...
    <div class="container">
        ...
        <!-- /.row -->
         ...
         <!-- Modal Update Mutasi-->
      <form id="add-row-form" action="<?php echo site_url('admin/mutasi/edit_barang');?>" method="post">
               ...
               <div class="modal-content">
                   ...
                   <div class="modal-body">
                       <div class="form-group">
                           <label for="barang_imei">IMEI :</label>
                           <input type="text" name="barang_imei" class="form-control" readonly>
                       </div>
                       <div class="form-group">
                           <label for="barang_imei">Nama Barang :</label>
                           <input type="text" name="barang_nama" class="form-control" readonly>
                       </div>
                       <div class="form-group">
                           <label for="barang_imei">Toko Awal :</label>
                           <input type="text" name="toko_awal" class="form-control" readonly>
                       </div>
                       <div class="form-group">
                           <label for="barang_imei">Toko Tujuan :</label>
                            <select name="toko_tujuan" class="form-control" required>
                                <?php foreach ($kat4->result_array() as $row) :?>
                                    <?php if($row['toko_id']==$row['toko_id']) ?>
                                    <option value="<?= $row['toko_id'];?>"><?= $row['toko_nama'];?></option>
                                <?php endforeach;?>
                            </select>
                       </div>
                   </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(e){
            ...
            $('#mydata').DataTable({
                ...
                "order": [[0, "asc" ]],
                "ajax":{
                    url :  base_url+'admin/mutasi/showBarang',
                    type : 'POST'
                },
                columns: [
                    ...
                    { data: "3" },
                    { data: "4" },
                    { data: "5", render: function (data) { return data == 1 ? "ACC" : "HP"; }},
                    { data: "6", render: function (data, type, row) { if (data == 1) { return row['3'] == 0 ? "Sold" : "Avialable"; } return "Retur"}},
                    { data: "7" },
                ],
                // dom: 'Bfrtip',
                // buttons: [
            ...
            $('#mydata').on('click','.edit_record',function(){
                var imei=$(this).data('brgimei');
                var nama=$(this).data('brgnama');
                var tkAwal=$(this).data('tokonama');
                $('#ModalUpdate').modal('show');
                $('[name="barang_imei"]').val(imei);
                $('[name="barang_nama"]').val(nama);
                $('[name="toko_awal"]').val(tkAwal);
            });
